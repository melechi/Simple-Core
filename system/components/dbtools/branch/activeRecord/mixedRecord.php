<?php
class dbtools_activeRecord_mixedRecord extends dbtools_activeRecord_record
{
	...
	public function __construct($parents=array(),$record=array())
	{
		if (is_array($parents))
		{
			...
			{
				if ($parents[$i] instanceof dbtools_activeRecord_activeRecord)
				{
					$this->parent[]=$parents[$i];
				}
				else
				{
					throw new Exception('Invalid parent object. Parent object must be an instance of "dbtools_activeRecord_activeRecord".');
				}
			}
		}
		else
		{
			throw new Exception('Invalid argument. Parent argument must be an array of "dbtools_activeRecord_activeRecord" instances.');
		}
		if (is_array($record))
		{
			if (count($records)==$this->numRecords)
			{
				...
				for ($i=0; $i<$this->numRecords; $i++)
				{
					$this->initialRow[]	=$record[$i];
					$this->row[]		=$record[$i];
				}
			}
			else
			{
				throw new Exception('Invalid record count. Expected "'.$this->numRecords.'" records.');
			}
		}
		else
		{
