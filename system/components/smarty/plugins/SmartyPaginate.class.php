...
class SmartyPaginate {
    ...
    function assign(&$smarty, $var = 'paginate', $id = 'default') {
		/*
		 * README
		 * Dean Rather 2008-03-20: I've changed
		 * get_class($smarty)) == 'smarty'
		 * below to
		 * get_class($smarty)) == 'component_smarty'
		 * To make it compatible with SimpleCore.
		 */
        if(is_object($smarty) && (strtolower(get_class($smarty)) == 'component_smarty' || is_subclass_of($smarty, 'smarty'))) {
